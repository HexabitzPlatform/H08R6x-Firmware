...
void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOA, SYNC_Pin | AVDD_EN_Pin | LPn_Pin, GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB, CORE1_8_EN_Pin | LED_Pin | NCS_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pins : PAPin PAPin PAPin PAPin */
	GPIO_InitStruct.Pin = SPI_I2C_N_Pin | SYNC_Pin | AVDD_EN_Pin | LPn_Pin;
	...
	GPIO_InitStruct.Pin = CORE1_8_EN_Pin | LED_Pin | NCS_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
uint8_t IsFactoryReset(void) {
	...
	GetPortGPIOs(P_LAST, &P_last_TX_Port, &P_last_TX_Pin, &P_last_RX_Port,
			&P_last_RX_Pin);
	...
	Delay_ms_no_rtos(5);
	if (HAL_GPIO_ReadPin((GPIO_TypeDef*) P_last_RX_Port, P_last_RX_Pin)
			== RESET) {
